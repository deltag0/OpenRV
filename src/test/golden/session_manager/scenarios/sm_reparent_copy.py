"""Scenario: reparent (COPY) outcome — the graph result of dragging a node into a
second folder with CopyAction (COVERAGE §G2): the node is added to the new folder and
KEPT in the old one (multi-parent), unlike the MOVE case (`sm_reparent`).

Outcome pinned via the equivalent `rv.commands` (real drag can't be driven headless, §G).
"""
...
# COPY: add to FolderB, keep in FolderA (multi-parent outcome).
