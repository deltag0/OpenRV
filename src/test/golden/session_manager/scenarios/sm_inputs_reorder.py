"""Scenario: session_manager Inputs tab -- pinned NON-sorted input order (E3/E7).

Same three-source RVStackGroup as ``sm_inputs``, but the source GROUP nodes
are wired via setNodeInputs in a deliberately NON-sorted order
(``[srcC, srcA, srcB]`` = ``[002, 000, 001]``). This pins the connection
ORDER in the graph so an input-reorder regression in the port is caught.
...
srcA, srcB, srcC = group_nodes  # 000000, 000001, 000002
reordered = [srcC, srcA, srcB]
log("reordered inputs:", reordered)
