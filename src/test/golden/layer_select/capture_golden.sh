#!/usr/bin/env bash
# Capture Mu baselines for layer_select golden scenarios into golden/<id>/
#
# Usage:
...
if [ ${#ids[@]} -eq 0 ]; then
    echo "Nothing to capture (all goldens present, skipped, or missing LAYER_EXR_FIXTURE)."
    exit 0
fi
...
echo "Capturing ${#ids[@]} layer_select scenario(s) with --impl mu, 2x determinism check"
...
for id in "${ids[@]}"; do
    ...
    if ! python3 "$RUNNER" \
        ...
        echo "FAIL $id (run_scenario run 1)"
    ...
    if ! python3 "$RUNNER" \
        ...
        --allow-runtime-errors \
        "${RUNNER_MODE[@]}" $menu_bar_flag; then
        echo "FAIL $id (run_scenario run 2)"
        fail=$((fail + 1)); fail_list="$fail_list $id"
        continue
    fi
    ...
    for png in "$out1"/*.png; do
        ...
        name="$(basename "$png")"
        if [ ! -f "$out2/$name" ]; then
            echo "FAIL $id: $name missing from run 2"
            det_ok=0
            continue
        fi
...
echo "---"
if [ "$fail" -gt 0 ]; then
    echo "Capture incomplete -- FAILED:$fail_list"
    exit 1
fi
