...
try:
    ...
    from PySide6.QtCore import Qt, QFile, QModelIndex, QSize, QTimer
    ...
    from PySide2.QtCore import Qt, QFile, QModelIndex, QSize, QTimer
...
class FilmstripWidget(QLabel):
    ...
    def showFrameAtX(self, mouse_x: int) -> None:
        ...
        frame = self._strip.copy(QtCore.QRect(frame_x, 0, self._frame_width, self._strip.height()))
...
class SessionManagerMode(SessionManagerInteractions, rvtypes.MinorMode):
    ...
    def __init__(self):
        ...
        if previews_env is not None and previews_env == "0":
            ...
            try:
                self._previews_enabled = bool(commands.readSettings("SessionManager", "previewsEnabled", True))
            except Exception:
                pass
    ...
    def _build_panel(self) -> None:
        ...
        if tree_view_base is None or inputs_view_base is None:
            print("session_manager: UI missing treeView or inputsListView in %s" % ui_path)
            return
    ...
    def _cache_icons(self) -> None:
        ...
        self._type_icons = [(t, _aux_icon(icon, True, self._dark_ui)) for t, icon in type_icon_names]
        self._view_icon = _aux_icon("view.png", True, self._dark_ui)
        self._layer_icon = _aux_icon("layer.png", True, self._dark_ui)
        self._channel_icon = _aux_icon("channel.png", True, self._dark_ui)
        self._unknown_type_icon = _aux_icon("new_48x48.png", True, self._dark_ui)
    ...
    def _new_node_sub_component(
        ...
        if sub_component != MediaSubComponent:
            icon_path = ":images/radio_button_blue_on.png" if selected else ":images/radio_button_dark.png"
    ...
    def _populate_source_subcomponents(self, item: QStandardItem, node: str, parent: str) -> None:
    ...
    def _make_source_row_widget(self, node: str) -> QWidget:
        ...
        fallback_path = self.auxFilePath("fallback_thumbnail.png")
        fallback = QPixmap(fallback_path)
        if fallback.isNull():
            fallback = self._fallback_source_icon.pixmap(QSize(SOURCE_PREVIEW_WIDTH, SOURCE_PREVIEW_HEIGHT))
    ...
    def update_tree(self) -> None:
        ...
        try:
            ...
            for cat_item in category_items:
                ...
                self._view_tree_view.setExpanded(self._view_model.indexFromItem(cat_item), expanded)
    ...
    def _node_inputs_changed(self, event) -> None:
        ...
        if node == commands.viewNode():
            self.update_inputs(node)
        if commands.nodeType(node) == "RVFolderGroup" and self._view_tree_view._drop_action == Qt.IgnoreAction:
