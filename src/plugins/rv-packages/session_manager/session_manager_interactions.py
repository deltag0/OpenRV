...
try:
    ...
    from PySide2.QtGui import QAction, QDragEnterEvent, QDropEvent, QIcon, QStandardItem
...
class SessionManagerInteractions:
    ...
    def _setup_toolbar_and_menus(self) -> None:
        ...
        add_action = QAction(_aux_icon("add_48x48.png"), "Create View", add_button)
        folder_action = QAction(_aux_icon("foldr_48x48.png"), "Create Folder", folder_button)
        delete_action = QAction(_aux_icon("trash_48x48.png"), "Delete View", delete_button)
        config_action = QAction(_aux_icon("confg_48x48.png"), "Configure", config_button)
        edit_info_action = QAction(_aux_icon("sinfo_48x48.png"), "Edit View Info", edit_info_button)
        order_up_action = QAction(_aux_icon("up_48x48.png"), "Move Input Higher in List", order_up)
        order_down_action = QAction(_aux_icon("down_48x48.png"), "Move Input Lower in List", order_down)
        sort_asc_action = QAction("A-Z", sort_asc)
        sort_desc_action = QAction("Z-A", sort_desc)
        inputs_delete_action = QAction(_aux_icon("trash_48x48.png"), "Delete Input", inputs_delete)
        prev_view_action = QAction(_aux_icon("back_48x48.png"), "Previous View", self._prev_view_button)
        next_view_action = QAction(_aux_icon("forwd_48x48.png"), "Next View", self._next_view_button)
        home_action = QAction(_aux_icon("home_48x48.png"), "Select Current View", home_button)
        ...
        add_sequence = add_menu.addAction(_aux_icon("playlist_48x48.png"), "Sequence")
        add_stack = add_menu.addAction(_aux_icon("photoalbum_48x48.png"), "Stack")
        add_switch = add_menu.addAction(_aux_icon("shuffle_48x48.png"), "Switch")
        add_folder = add_menu.addAction(_aux_icon("foldr_48x48.png"), "Folder")
        add_layout = add_menu.addAction(_aux_icon("lgicn_48x48.png"), "Layout")
        add_retime = add_menu.addAction(_aux_icon("tempo_48x48.png"), "Retime")
        add_colorize = add_menu.addAction(_aux_icon("new_48x48.png"), "Color")
        add_ocio = add_menu.addAction(_aux_icon("new_48x48.png"), "OCIO")
        add_dynamic = None
        if os.getenv("RV_ENABLE_DYNAMIC_NODE") is not None:
            add_dynamic = add_menu.addAction(_aux_icon("new_48x48.png"), "Dynamic")
        add_user_node = add_menu.addAction(_aux_icon("new_48x48.png"), "New Node by Type...")
        add_menu.addSeparator()
        add_srgb = add_menu.addAction(_aux_icon("colorchart_48x48.png"), "SRGB Color Chart...")
        add_aces = add_menu.addAction(_aux_icon("colorchart_48x48.png"), "ACES Color Chart...")
        add_cbars = add_menu.addAction(_aux_icon("ntscbars_48x48.png"), "Color Bars...")
        add_black = add_menu.addAction(_aux_icon("video_48x48.png"), "Black...")
        add_color = add_menu.addAction(_aux_icon("video_48x48.png"), "Color...")
        add_blank = add_menu.addAction(_aux_icon("video_48x48.png"), "Blank...")
    ...
    def _wire_interaction_signals(self) -> None:
        ...
        self._view_tree_view.doubleClicked.connect(
            lambda index: self._view_by_index(index, self._view_model)
        )
        self._view_tree_view.pressed.connect(
            lambda index: self._item_pressed(index, self._view_model)
        )
        self._inputs_view.doubleClicked.connect(
            lambda index: self._view_by_index(index, self._inputs_model)
        )
    ...
    def _add_node_by_type_name(self) -> None:
        if self._new_node_dialog is None:
            ...
            if label is not None:
                label.setPixmap(_aux_icon("new_48x48.png").pixmap(48, 48))
            self._new_node_dialog.accepted.connect(
                lambda: self._add_node_of_type(self._node_type_combo.currentText())
            )
    ...
    def _add_movie_proc(self, fmtspec: str) -> None:
        ...
        icon = _aux_icon("video_48x48.png")
