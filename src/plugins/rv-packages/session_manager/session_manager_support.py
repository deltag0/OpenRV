...
try:
    ...
    from PySide6.QtGui import QIcon, QImage, QPixmap, QStandardItem
except ImportError:  # pragma: no cover
    from PySide2.QtCore import Qt
    from PySide2.QtGui import QIcon, QImage, QPixmap, QStandardItem
...
def color_adjusted_icon(rpath: str, dark_ui: bool, invert_sense: bool = False) -> QIcon:
    """Match Mu ``colorAdjustedIcon`` (session_manager.mu.in ~1153)."""
    icon0 = QImage(rpath.replace("48x48", "out"))
    icon1 = QImage(rpath)
    swap = invert_sense != dark_ui
    qimage = icon0 if swap else icon1
    icon = QIcon(QPixmap.fromImage(qimage, Qt.AutoColor))
    icon.addPixmap(QPixmap.fromImage(icon1, Qt.AutoColor), QIcon.Selected, QIcon.Off)
    return icon


def aux_icon(name: str, color_adjust: bool = False, dark_ui: bool = True) -> QIcon:
    """Match Mu ``auxIcon`` — ``:images/`` qrc paths with optional dark-UI swap."""
    rpath = ":images/" + name
    if color_adjust:
        return color_adjusted_icon(rpath, dark_ui, False)
    return QIcon(rpath)
...
def hashed_sub_component(media: str, view: str | None, layer: str | None) -> str:
    v = "@." if view == "" else view
    layer_part = "@." if layer == "" else layer
    if v is None and layer_part is None:
        return "%s!~!~" % media
    if v is None:
        return "%s!~%s!~" % (media, layer_part)
    if layer_part is None:
        return "%s!~!~%s" % (media, v)
    return "%s!~%s!~%s" % (media, v, layer_part)
